...
train_data = np.array(train_sequences).reshape((-1, 60, 10))
valid_data = np.array(valid_sequences).reshape((-1, 60, 10))
test_data = np.array(test_sequences).reshape((-1, 60, 10))
...
#         self.W = self.add_weight(shape=(input_shape[-1], self.hidden_dim), initializer = 'he_normal', trainable=True, name='w1')
#         self.bw = self.add_weight(shape=(self.hidden_dim,), initializer = 'zero', trainable=True, name='w2')
#         self.uw = self.add_weight(shape=(self.hidden_dim,), initializer = 'he_normal', trainable=True, name='w3')
...
# preds = Dense(9, activation='softmax')(l_drop_sent)
...
embedded_sequences = embedding_layer(sentence_input)
l_gru = Bidirectional(
    GRU(n_hidden, return_sequences=True, kernel_initializer='he_normal', recurrent_initializer='he_normal',
        activation='sigmoid'),
...
review_input = Input(shape=(MAX_SONG_LENGTH,MAX_LINE_LENGTH), name='review_input')
